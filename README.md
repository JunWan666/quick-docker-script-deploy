...
```bash
curl -fsSL https://raw.githubusercontent.com/JunWan666/quick-docker-script-deploy/main/one-click/deploy.sh | bash
```

也可以直接进入指定功能，例如先安装 Docker：

```bash
curl -fsSL https://raw.githubusercontent.com/JunWan666/quick-docker-script-deploy/main/one-click/deploy.sh | bash -s -- docker
```

如果你在局域网里临时提供脚本文件，也可以使用局域网地址：

```bash
curl -fsSL http://192.168.11.6:7878/one-click/deploy.sh | bash
```

如果想先保存脚本再执行，仍然可以下载到 `/tmp/deploy.sh` 后运行。
如果当前环境不支持从 `/dev/tty` 读取交互输入，也建议使用保存后执行的方式。
